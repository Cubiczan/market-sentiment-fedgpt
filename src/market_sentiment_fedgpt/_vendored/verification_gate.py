"""Vendored copy of the canonical VerificationGate — do NOT edit freely.
...
library" (README, tech-stack table); a mandatory runtime dependency would
break that claim. Vendoring keeps the stdlib-only posture while still
ending the -10/-12 drift.

Keep-in-sync rule: this file must ONLY be advanced by a reviewed PR that
bumps BOTH the Upstream commit stamp above and cites the upstream change
(or explains the local delta). Migration condition for deleting this
copy entirely: if fedgpt ever retires the zero-dependency/air-gap
posture, replace this module with the hash-pinned cubiczan-resilience
dependency (see hedge-fund-13f-radar for that pattern) and import from
cubiczan_resilience.verification_gate.
